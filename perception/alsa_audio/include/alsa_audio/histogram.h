...
template<class T, class TOut = double>
  ...
  public:
     ...
     */
    void SetMinSpectrum(const T& tMin)
    {
      m_tMin = tMin;
      ComputeStep();
    }
    ...
    /*! This function sets the minimum of the histogram spectrum.
     The spectrum is not recomputed, use it with care
     */
    void SetMaxSpectrum(const T& tMax)
    {
      m_tMax = tMax;
      ComputeStep();
    }
    ...
    const T& GetMinSpectrum() const
    {
      return m_tMin;
    }
    ;
    //! return maximum of histogram spectrum
    const T& GetMaxSpectrum() const
    {
      return m_tMax;
    }
    ;
     ...
     */
    static void GetMoments(const std::vector<T>& vData,
                           TOut& fAve,
                           TOut& fAdev,
                           TOut& fSdev,
                           TOut& fVar,
    ...
    {
      return m_vCounters.size();
    }
    ;
    //! returns i-th counter
    unsigned int operator [](unsigned int i) const
    {
      assert(i < m_vCounters.size());
      return m_vCounters[i];
    }
    ;
  ...
  protected:
    ...
    void ComputeStep()
    {
      m_dStep = (TOut)(((TOut)(m_tMax - m_tMin)) / (m_vCounters.size() - 1));
    }
    ;
    //! Data accumulators
...
template<class T, class TOut>
  TOut THistogram<T, TOut>::GetArea() const
  {
    ...
    if (n > 6)
    {
      area = 3.0 / 8.0 * (m_vCounters[0] + m_vCounters[n - 1]) + 7.0 / 6.0 * (m_vCounters[1] + m_vCounters[n - 2])
    ...
    {
      area = 5.0 / 12.0 * (m_vCounters[0] + m_vCounters[n - 1]) + 13.0 / 12.0 * (m_vCounters[1] + m_vCounters[n - 2]);
...
template<class T, class TOut>
  void THistogram<T, TOut>::GetMoments(const std::vector<T>& vData,
                                       TOut& fAve,
                                       TOut& fAdev,
                                       TOut& fSdev,
                                       TOut& fVar,
