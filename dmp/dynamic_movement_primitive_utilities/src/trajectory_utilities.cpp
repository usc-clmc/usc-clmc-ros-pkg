                                                 ...
                                                 const bool compute_derivatives)
{
  ...
  if ((NUM_FORCES != 6) && (NUM_FORCES != 12))
  {
    ROS_ERROR("Number of wrench variable names >%i< is invalid, cannot create wrench trajectory from bag file >%s<.",
  ...
  if (!usc_utilities::FileIO<WrenchStampedMsg>::readFromBagFile(wrench_state_msgs, topic_name, abs_bag_file_name, false))
    return false;
  ...
  if (!trajectory.initialize(wrench_variable_names, sampling_frequency, true, NUM_DATA_POINTS))
    return false;
  ...
  ROS_ASSERT(right_arm_force_variable_names.size() == 6);
  ...
  ROS_ASSERT(left_arm_force_variable_names.size() == 6);
  unsigned int right_arm_force_variables_matches = 0;
  unsigned int left_arm_force_variables_matches = 0;
  for (unsigned int i = 0; i < NUM_FORCES; ++i)
  {
    for (unsigned int j = 0; j < right_arm_force_variable_names.size(); ++j)
    {
      if(right_arm_force_variable_names[j].compare(wrench_variable_names[i]) == 0)
      {
        right_arm_force_variables_matches++;
      }
    }
      ...
      && (right_arm_force_variables_matches + left_arm_force_variables_matches != NUM_FORCES))
  {
    for (unsigned int i = 0; i < NUM_FORCES; ++i)
    {
      ROS_ERROR("Wrench variable name >%s< does not match. Cannot create wrench trajectory.", wrench_variable_names[i].c_str());
    }
    return false;
  }
                                                       ...
                                                       const bool compute_derivatives)
{
  ...
  // initialize trajectory
  // TODO: using sampling_frequency, which actually is not required.
  if (!trajectory.initialize(acceleration_variable_names, sampling_frequency, true, NUM_DATA_POINTS))
    return false;
  vector<ros::Time> time_stamps;
  ...
  ROS_ASSERT(left_hand_acceleration_variable_names.size() == 3);
  unsigned int right_hand_acceleration_variables_matches = 0;
  unsigned int left_hand_acceleration_variables_matches = 0;
                                                     ...
                                                     const bool compute_derivatives)
{
  ...
  if (!usc_utilities::FileIO<JointStateMsg>::readFromBagFile(joint_state_msgs, topic_name, abs_bag_file_name, false))
    return false;
  ROS_DEBUG("Read >%i< joint messages from bag file >%s<.", (int)joint_state_msgs.size(), abs_bag_file_name.c_str());

  const unsigned int NUM_JOINTS = joint_variable_names.size();
  const unsigned int NUM_DATA_POINTS = joint_state_msgs.size();
  VectorXd joint_positions = VectorXd::Zero(NUM_JOINTS);

  // initialize trajectory
  // TODO: using sampling_frequency, which actually is not required.
  if (!trajectory.initialize(joint_variable_names, sampling_frequency, true, (int)NUM_DATA_POINTS))
    return false;
  vector<ros::Time> time_stamps;
  ...
  {
    int index = 0;
    unsigned int num_joints_found = 0;
    ...
    {
      ROS_ERROR("Number of joints is >%i<, but there have been only >%i< matches.", (int)NUM_JOINTS, num_joints_found);
      return false;
    }
    // add data
                                             ...
                                             const std::vector<std::string>& variable_names)
{
  ...
  if (pose_msgs.empty())
  {
    ROS_ERROR("No pose messages read from >%s< on topic >%s<.", abs_bag_file_name.c_str(), topic_name.c_str());
    return false;
  }
  ...
  if (!pose_trajectory.initialize(variable_names, sampling_frequency, true, NUM_DATA_POINTS))
    return false;
  ...
  if (offset.size() == usc_utilities::Constants::N_CART + usc_utilities::Constants::N_QUAT)
  {
    ...
    getEndeffectorPoseVector(pose_msgs.front(), offset);
...
//  Eigen::Vector4d last_quaternion = Eigen::Vector4d(pose_msgs[0].pose.orientation.w,
//                                                    pose_msgs[0].pose.orientation.x,
//                                                    pose_msgs[0].pose.orientation.y,
//                                                    pose_msgs[0].pose.orientation.z);
//  Eigen::Vector4d quat = Eigen::Vector4d::Zero();

  for (unsigned int i = 0; i < pose_msgs.size(); ++i)
  {
    time_stamps[i] = pose_msgs[i].header.stamp;
    getEndeffectorPoseVector(pose_msgs[i], endeffector_pose);

//    // flip quaternion according to previous quaternion
//    for (unsigned int j = 0; j < 4; ++j)
//    {
//      quat(j) = endeffector_pose[usc_utilities::Constants::N_CART + j];
//    }
//    if (last_quaternion.dot(quat) < 0.0)
//    {
//      quat = -quat;
//    }
//    for (unsigned int j = 0; j < 4; ++j)
//    {
//      endeffector_pose[usc_utilities::Constants::N_CART + j] = quat(j);
//    }
  ...
  ROS_ASSERT_MSG(time_stamps.back().toSec() - time_stamps.front().toSec() > 0, "Time stamps in >%s< are invalid.", abs_bag_file_name.c_str());
  return TrajectoryUtilities::resample(pose_trajectory, time_stamps, sampling_frequency, false);
}

void TrajectoryUtilities::fixQuaternionSign(const Eigen::Vector4d& q1, Eigen::Vector4d& q2)
{
  if (q1.dot(q2) < 0.0)
  {
    q2 = -q2;
                                               ...
                                               const std::vector<std::string>& variable_names)
{
  ...
  // Debugging...
  ROS_DEBUG("Creating pose trajectory from link >%s< to link >%s< with >%d< joints.",
            start_link_name.c_str(), end_link_name.c_str(), arm_chain.getNumJoints());
  std::vector<std::string> chain_joint_names;
  arm_chain.getJointNames(chain_joint_names);
  for (unsigned int i = 0; i < chain_joint_names.size(); ++i)
  {
    ROS_DEBUG("Joint >%i< is >%s<.", i, chain_joint_names[i].c_str());
  }

  // error checking
  if (!arm_joint_trajectory.isInitialized())
    return false;
  ROS_ASSERT(NUM_TRAJECTORY_POINTS > 0);
  ROS_ASSERT(TRAJECTORY_DIMENSION > 0);

  if (!pose_trajectory.initialize(variable_names, arm_joint_trajectory.getSamplingFrequency(), true, NUM_TRAJECTORY_POINTS))
    return false;
  ...
  if (offset.size() == usc_utilities::Constants::N_CART + usc_utilities::Constants::N_QUAT)
  {
    ...
    getEndeffectorPoseVector(endeffector_frame, offset);
    setInverseOffsetTransform(inverse_offset_transform, offset);
  }

  std::vector<double> endeffector_pose(usc_utilities::Constants::N_CART + usc_utilities::Constants::N_QUAT, 0.0);

  ROS_VERIFY(arm_joint_trajectory.getTrajectoryPosition(0, arm_kdl_joint_array.data));
  arm_chain.forwardKinematics(arm_kdl_joint_array, endeffector_frame);
  getEndeffectorPoseVector(endeffector_frame, endeffector_pose);
//  Eigen::Vector4d last_quaternion = Eigen::Vector4d(endeffector_pose[usc_utilities::Constants::N_CART + 0],
//                                                    endeffector_pose[usc_utilities::Constants::N_CART + 1],
//                                                    endeffector_pose[usc_utilities::Constants::N_CART + 2],
//                                                    endeffector_pose[usc_utilities::Constants::N_CART + 3]);
//  Eigen::Vector4d quat = Eigen::Vector4d::Zero();
...
void TrajectoryUtilities::getEndeffectorPoseVector(const KDL::Frame& kdl_frame, std::vector<double>& endeffector_pose)
{
  // std::vector<double> endeffector_pose(usc_utilities::Constants::N_CART + usc_utilities::Constants::N_QUAT, 0.0);
...
void TrajectoryUtilities::getEndeffectorPoseVector(const PoseStampedMsg& pose_msg, std::vector<double>& endeffector_pose)
{
  // std::vector<double> endeffector_pose(usc_utilities::Constants::N_CART + usc_utilities::Constants::N_QUAT, 0.0);
