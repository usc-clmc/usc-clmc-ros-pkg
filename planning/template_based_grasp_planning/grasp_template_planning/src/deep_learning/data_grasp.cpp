...
Data_grasp::Data_grasp(const std::vector<double> &pgripper_joints,const geometry_msgs::Pose &pgripper_pose,
		...
		gripper_joints(pgripper_joints),gripper_pose(pgripper_pose), grasp_template(pgrasp_template), uuid(
...
Data_grasp::Data_grasp(Data_grasp_log &d_grasp) :
		gripper_joints(d_grasp.gripper_joints.vals),gripper_pose(d_grasp.gripper_pose.pose), grasp_template(d_grasp.grasp_template_heightmap,d_grasp.grasp_template_pose.pose), uuid(d_grasp.uuid), success(d_grasp.success) {
}
