...
Data_grasp Data_loader::Load_grasp_template(const std::string &path_bagfile,const std::string &topic) {
	...
	Data_grasp result_grasp(gripper_pose_offset, g_temp);
	result_grasp.uuid = grasp_matched.uuid;
	return result_grasp;
}

bool Data_loader::Load_grasp_database(const std::string &path_bagfile, const std::string &topic,
...
bool Data_loader::Extract_dataset(const std::string &path_bagfile,const std::string &topic,
...
bool Data_loader::Load_trial_log(const std::string &path_bagfile,const std::string &topic,
		std::vector<Data_grasp> &result_grasp,
	...
	for (grasp_template::HsIterator it = heightmap_computation.getIterator();
		...
		extract_temp.Get_random_grasp_templates(t, random_templates,
				gripper_pose, gripper_pose_offset);
		for (unsigned int j = 0; j < random_templates.size(); ++j) {
			result_grasp.push_back(
					Data_grasp(gripper_pose[j], random_templates[j]));
		}
