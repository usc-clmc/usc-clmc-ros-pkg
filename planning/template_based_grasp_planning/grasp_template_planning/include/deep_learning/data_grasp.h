...
struct Data_grasp_log_ {
	...
	Data_grasp_log_() :
			stamp(), gripper_pose(), grasp_template_pose(), grasp_template_heightmap(), uuid(), success() {
	}

	Data_grasp_log_(const ContainerAllocator& _alloc) :
			stamp(), gripper_pose(_alloc), grasp_template_pose(_alloc), grasp_template_heightmap(
					_alloc), uuid(_alloc), success(_alloc) {
	}

  typedef boost::shared_ptr< ::deep_learning::Data_grasp_log_<ContainerAllocator> > Ptr;
  typedef boost::shared_ptr< ::deep_learning::Data_grasp_log_<ContainerAllocator>  const> ConstPtr;
...
public:
	...
	Data_grasp(const geometry_msgs::Pose &pgripper_pose,
...
template<typename Stream> static void stream(Stream& s,
	...
	Printer<
			std::basic_string<char, std::char_traits<char>,
					typename ContainerAllocator::template rebind<char>::other> >::stream(
			s, indent + "  ", v.uuid);
	s << indent << "stamp: ";
