...
template<class MessageType>
  bool TaskLabelerIO<MessageType>::getLastTrialId(const task_recorder2_msgs::Description description,
                                                  int& trial_id)
  {
    boost::filesystem::path absolute_data_directory_path = boost::filesystem::path(data_directory_name_ + task_recorder2_utilities::getFileName(description));
    if(!task_recorder2_utilities::readTrialCounterFile(absolute_data_directory_path, trial_id, labeled_file_name_))
    {
      ROS_ERROR("Could not read last trial id from directory >%s<.", absolute_data_directory_path.filename().c_str());
      return false;
    }
    return true;
  }
...
template<class MessageType>
  bool TaskLabelerIO<MessageType>::setLastTrialId(task_recorder2_msgs::Description& description)
{
  int trial_id;
  if(!getLastTrialId(description, trial_id))
  {
    ROS_ERROR("Could not set last trial id of description >%s<.", task_recorder2_utilities::getFileName(description).c_str());
    return false;
  }
  if(trial_id > 0)
  {
  ...
  {
    ROS_WARN("Setting last trial id to >%i<.", trial_id);
  }
