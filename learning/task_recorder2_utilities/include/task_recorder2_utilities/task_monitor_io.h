...
template<class MessageType, class MessageTypeLabel>
  ...
  typedef boost::shared_ptr<MessageTypeLabel const> MessageTypeLabelConstPtr;
     ...
     * @return True on success, otherwise False
     */
    bool getList(const std::string description_directory,
...
template<class MessageType, class MessageTypeLabel>
                                                              ...
                                                              MessageTypeLabel& data_label)
  {
    ...
    ROS_INFO_COND(data_label.type == data_label.BINARY_LABEL, "Read >%i< data samples and 1 binary label (%i) from (trial:%i) >%s<.",
                  (int)data_samples.size(), data_label.binary_label.label, description.trial, file_name.c_str());
    ROS_INFO_COND(data_label.type == data_label.COST_LABEL, "Read >%i< data samples and 1 cost label (%f) from (trial:%i) >%s<.",
                  (int)data_samples.size(), data_label.cost_label.cost, description.trial, file_name.c_str());
    return true;
  }
...
template<class MessageType, class MessageTypeLabel>
  bool TaskMonitorIO<MessageType, MessageTypeLabel>::getList(std::vector<std::string>& descriptions)
  {
...
template<class MessageType, class MessageTypeLabel>
  bool TaskMonitorIO<MessageType, MessageTypeLabel>::getList(const std::string description_directory,
                                                             std::vector<task_recorder2_msgs::Description>& labeled_descriptions,
                                                             std::vector<task_recorder2_msgs::Description>& unlabeled_descriptions)
  {
    ROS_ASSERT_MSG(initialized_, "Task labeler IO module is not initialized.");
    ...
    // get number of trials
    boost::filesystem::path absolute_data_directory_path = boost::filesystem::path(data_directory_name_ + description_directory);
    ROS_VERIFY(checkForDirectory(absolute_data_directory_path, false));

    int number_of_trials;
    ROS_VERIFY(getTrialId(absolute_data_directory_path, number_of_trials, labeled_file_name_));
    ROS_DEBUG("There are >%i< trials.", number_of_trials);

    std::string description_string;
    int id = 0;
    ROS_VERIFY(task_recorder2_utilities::parseDescriptionString(description_directory, description_string, id));
    task_recorder2_msgs::Description description;
    description.description = description_string;
    description.id = id;

    labeled_descriptions.clear();
    unlabeled_descriptions.clear();
    for (int i = 0; i < number_of_trials; ++i)
    {
      std::string file_name = getPathNameIncludingTrailingSlash(absolute_data_directory_path);
      file_name.append(getDataFileName(labeled_file_name_, i));

      if (usc_utilities::FileIO<task_recorder2_msgs::DataSample>::containsMsg(labeled_file_name_, file_name))
      {
        description.trial = i;
        if (usc_utilities::FileIO<task_recorder2_msgs::DataSample>::containsMsg(topic_name_, file_name))
        {
          labeled_descriptions.push_back(description);
        }
        else
        {
          unlabeled_descriptions.push_back(description);
        }
      }
    }
    return true;
