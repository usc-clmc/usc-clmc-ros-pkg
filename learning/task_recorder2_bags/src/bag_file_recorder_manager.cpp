...
#include <task_recorder2_bags/bag_file_recorder_manager.h>
...
bool BagFileRecorderManager::initialize(ros::NodeHandle node_handle)
{
  ...
  for (unsigned int i = 0; i < topic_names.size(); ++i)
  {
    ...
    {
      boost::shared_ptr<task_recorder2_bags::BagFileRecorderClient<tf::tfMessage, task_recorder2_msgs::tfMessageStamped, task_recorder2_bags::TFHeaderExtractor<tf::tfMessage> > > recorder;
      recorder.reset(new task_recorder2_bags::BagFileRecorderClient<tf::tfMessage, task_recorder2_msgs::tfMessageStamped, task_recorder2_bags::TFHeaderExtractor<tf::tfMessage> >(topic_names[i]));
      clients_.push_back(recorder);
    }
    else
    {
      ROS_ERROR("Unknown recorder client >%s<.", topic_names[i].c_str());
      return false;
    }
  }
