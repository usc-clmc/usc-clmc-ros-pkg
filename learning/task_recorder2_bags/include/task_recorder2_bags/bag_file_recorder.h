...
template<class MessageType, class MessageTypeStamped>
   ...
   * @param node_handle
   * @param topic_name
   * @param header_utility
   * @return True on success, otherwise False
   */
  bool initialize(ros::NodeHandle node_handle, const std::string& topic_name,
                  const boost::shared_ptr<task_recorder2_bags::HeaderUtility<MessageType> > header_utility);
  ...
  private:
    ...
    /*!
     */
    task_recorder2_io::TaskRecorderIO<MessageTypeStamped> recorder_io_;
