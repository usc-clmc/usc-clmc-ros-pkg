#ifndef BOOST_ATOMIC_DETAIL_ATOMIC_BUILDER_HPP
#define BOOST_ATOMIC_DETAIL_ATOMIC_BUILDER_HPP
...
- load(memory_order2 order)
- compare_exchange_weak(integral_type &expected, integral_type desired, memory_order2 order)
...
public:
	...
	integral_type exchange(integral_type replacement, memory_order2 order=memory_order2_seq_cst) volatile
	{
		integral_type o=load(memory_order2_relaxed);
		do {} while(!compare_exchange_weak(o, replacement, order, memory_order2_relaxed));
		return o;
	}
...
- fetch_add_var(integral_type c, memory_order2 order)
- fetch_inc(memory_order2 order)
- fetch_dec(memory_order2 order)
...
public:
	...
	integral_type fetch_add(
		integral_type c, memory_order2 order=memory_order2_seq_cst) volatile
	{
		integral_type o=Base::load(memory_order2_relaxed), n;
		do {n=o+c;} while(!compare_exchange_weak(o, n, order, memory_order2_relaxed));
		return o;
	}
...
- fetch_add(integral_type c, memory_order2 order)
...
public:
	...
	integral_type fetch_and(integral_type c, memory_order2 order=memory_order2_seq_cst) volatile
	{
		integral_type o=load(memory_order2_relaxed), n;
		do {n=o&c;} while(!compare_exchange_weak(o, n, order, memory_order2_relaxed));
		return o;
	}
	integral_type fetch_or(integral_type c, memory_order2 order=memory_order2_seq_cst) volatile
	{
		integral_type o=load(memory_order2_relaxed), n;
		do {n=o|c;} while(!compare_exchange_weak(o, n, order, memory_order2_relaxed));
		return o;
	}
	integral_type fetch_xor(integral_type c, memory_order2 order=memory_order2_seq_cst) volatile
	{
		integral_type o=load(memory_order2_relaxed), n;
		do {n=o^c;} while(!compare_exchange_weak(o, n, order, memory_order2_relaxed));
		return o;
...
- load(memory_order2 order)
- store(integral_type i, memory_order2 order)
- compare_exchange_weak(integral_type &expected, integral_type desired, memory_order2 order)
...
- store(integral_type i, memory_order2 order)
- compare_exchange_weak(integral_type &expected, integral_type desired, memory_order2 order)
- compare_exchange_strong(integral_type &expected, integral_type desired, memory_order2 order)
- exchange(integral_type replacement, memory_order2 order)
- fetch_add_var(integral_type c, memory_order2 order)
- fetch_inc(memory_order2 order)
- fetch_dec(memory_order2 order)
...
public:
	...
	build_base_from_larger_type(integral_type t) {store(t, memory_order2_relaxed);}
		...
		memory_order2 failure_order) volatile
	{
		...
		expected_=get_base().load(memory_order2_relaxed);
		...
		memory_order2 order=memory_order2_seq_cst) volatile
	{
		...
		expected=get_base().load(memory_order2_relaxed);
		do {
			desired=insert(expected, v);
		} while(!get_base().compare_exchange_weak(expected, desired, order, memory_order2_relaxed));
	}
