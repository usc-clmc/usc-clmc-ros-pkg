#ifndef BOOST_ATOMIC_DETAIL_ATOMIC_GENERIC_CAS_HPP
#define BOOST_ATOMIC_DETAIL_ATOMIC_GENERIC_CAS_HPP
...
#if defined(__GNUC__)
	namespace boost_atomic { namespace detail { namespace atomic {
...
public:
	...
	{
		T expected=(T)i;
		do { } while(!const_cast<this_type *>(this)->compare_exchange_weak(expected, expected, order, memory_order2_relaxed));
		return expected;
	}
	...
	{
		T expected=(T)i;
		do { } while(!compare_exchange_weak(expected, r, order, memory_order2_relaxed));
		return expected;
	}
